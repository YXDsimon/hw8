...
SC_MODULE(Stimulus)
{
	...
	void main(void)
	{
		...
		for (i = 0; i < IMG_NUM; i++)
		{
			...
			cout << start << " ms: Stimulus sent frame " << i + 1 << "." << endl;
			TimeOut.write(start);
		}
	}
...
SC_MODULE(Gaussian_Smooth)
{
	...
	SC_CTOR(Gaussian_Smooth)
		: img_q("img_q", 1), knl_q1("knl_q1", 1), knl_q2("knl_q2", 1), tmp_q("tmp_q", 1), int_q1("int_q1", 1), int_q2("int_q2", 1), receiver("receiver"), g_kernel("g_kernel"), blurx("blurx"), blury("blury")
	{
...
SC_MODULE(DUT)
{
	...
	SC_CTOR(DUT)
		: q1("q1", 1), q2("q2", 1), q3("q3", 1), q4("q4", 1), q5("q5", 1), q6("q6", 1), q7("q7", 1), nms("nms", 1), gaussian_smooth("gaussian_smooth"), derivative_x_y("derivative_x_y"), magnitude_x_y("magnitude_x_y"), non_max_supp("non_max_supp"), apply_hysteresis("apply_hysteresis")
	{
